...
class KeyHandler: NSObject {
    ...
    enum FlagAsInt: Int64, CaseIterable {
        case command = 57344, shift, alt, control, fn, capslock
    ...
    typealias EventTuple = (key: KeyHandler.Event.EventType, sequence: KeySequence, cb: KeyHandler.Callback)
    var registeredEvents: [KeyHandler.Event.EventType: [(sequence: KeySequence, cb: KeyHandler.Callback)]] = [:]
    ...
    override init() {
        ...
        guard let eventTap = CGEvent.tapCreate(tap: .cgSessionEventTap, place: .headInsertEventTap, options: .defaultTap, eventsOfInterest: eventMask, callback: tapCallback, userInfo: selfPointer) else {
    ...
    func keyHandler(type: CGEventType, evt: CGEvent) -> CGEvent? {
        ...
        if type == .flagsChanged {
            let checkedFlags: [CGEventFlags] = [.maskCommand, .maskShift, .maskAlternate, .maskControl, .maskSecondaryFn, .maskAlphaShift]
    ...
    func addEventListener(key: KeyHandler.Event.EventType, sequence: KeySequence, _ cb: @escaping KeyHandler.Callback) {

...
class MenuBar: NSMenu {
    ...
    init() {
        ...
        let titleItem = NSMenuItem(title: "QuickSwitcher v\(Bundle.main.infoDictionary!["CFBundleShortVersionString"]!)", action: nil, keyEquivalent: "")
        self.addItem(titleItem)
        self.addItem(.separator())

        let openPrefsItem = NSMenuItem(title: "Preferences", action: #selector(self.openPrefs), keyEquivalent: "")
        openPrefsItem.target = self
        self.addItem(openPrefsItem)

        let closeItem = NSMenuItem(title: "Quit", action: #selector(self.closeApp), keyEquivalent: "")
